...
// import { submitExamResult } from '../utils/api';
// import { 
//   Clock, 
//   ChevronLeft, 
//   ChevronRight, 
...
//       // Submit to backend
//       const res = await submitExamResult(examData._id, payload, token);
//       onExamComplete(res.data.result || res.data);
//     } catch (err) {
//       let message = 'Failed to submit exam. Please try again.';
...
//       }
//       alert(message);
//       // Optionally fallback to local result
...
import { useState, useEffect } from "react";
import { submitExamResult } from "../utils/api";
import { ChevronLeft, ChevronRight, Flag, CheckCircle, Clock } from "lucide-react";
...
export default function ExamInterface({ examData, onExamComplete, onBack }) {
  ...
  const toggleFlag = (qIndex) => {
    setFlagged((prev) =>
      prev.includes(qIndex) ? prev.filter((x) => x !== qIndex) : [...prev, qIndex]
    );
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      onExamComplete(res.data.result);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="w-72 bg-white border-r p-6">
        ...
        <div className="flex items-center gap-2 text-sm mb-4">
          <Clock className="w-4 h-4 text-gray-600" />
          <span className="font-bold text-violet-600 text-lg">{formatTime(timeRemaining)}</span>
        </div>
        ...
        <button
          ...
          className="mt-6 w-full bg-violet-600 text-white py-3 rounded-lg font-semibold"
        >
          Submit Exam
        </button>
      </div>
      ...
      <div className="flex-1 p-8">
        <button onClick={onBack} className="text-gray-600 mb-4 hover:text-gray-900">
          ← Back
        </button>

        <h1 className="text-xl font-bold mb-2">
          {examData.skillName} • {examData.difficulty}
        </h1>
        ...
        <div className="bg-white p-6 rounded-xl border shadow-md">
          ...
          <div className="flex justify-between mt-6">
            <button
              onClick={() => setCurrentQuestion(Math.max(0, currentQuestion - 1))}
              disabled={currentQuestion === 0}
              className="text-gray-600 flex items-center gap-2 disabled:opacity-40"
            >
